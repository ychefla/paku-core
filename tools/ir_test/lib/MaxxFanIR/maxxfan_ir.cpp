...
void maxxfan_ir_send(const MaxxFanState& state) {
    ...
    // Build a flat bit stream first (level + duration_us per bit),
    // then pack pairs into rmt_item32_t to avoid duration1=0 glitches.
    //
    // Each byte: 1 start bit (mark=carrier ON) + 8 data bits LSB-first
    //            + 2 stop bits (space=carrier OFF)  = 11 bits/byte
    // 16 bytes * 11 bits = 176 bits + 1 final long space = 177 entries
    
    const size_t MAX_BITS = 200;
    uint8_t  bit_level[MAX_BITS];
    uint32_t bit_dur[MAX_BITS];
    size_t   nbit = 0;

    for (size_t byte_idx = 0; byte_idx < sizeof(packet); byte_idx++) {
        uint8_t byte_val = packet[byte_idx];

        // Start bit: mark (carrier ON)
        bit_level[nbit] = 1;
        bit_dur[nbit]   = BIT_TIME_US;
        nbit++;

        // 8 data bits, LSB first
        for (uint8_t b = 0; b < 8; b++) {
            bool one = (byte_val >> b) & 0x01;
            bit_level[nbit] = one ? 0 : 1;  // 1=space(OFF), 0=mark(ON)
            bit_dur[nbit]   = BIT_TIME_US;
            nbit++;
        }

        // 2 stop bits: space (carrier OFF), combined as one 2x-duration entry
        bit_level[nbit] = 0;
        bit_dur[nbit]   = BIT_TIME_US * 2;
        nbit++;
    }

    // Final long space
    bit_level[nbit] = 0;
    bit_dur[nbit]   = BIT_TIME_US * 8;
    nbit++;

    // Pack into rmt_item32_t pairs (level0/dur0 + level1/dur1)
    const size_t max_items = (MAX_BITS + 1) / 2 + 1;
    rmt_item32_t items[max_items];
    size_t item_idx = 0;

    for (size_t i = 0; i < nbit; i += 2) {
        items[item_idx].level0    = bit_level[i];
        items[item_idx].duration0 = us_to_rmt_ticks(bit_dur[i]);
        if (i + 1 < nbit) {
            items[item_idx].level1    = bit_level[i + 1];
            items[item_idx].duration1 = us_to_rmt_ticks(bit_dur[i + 1]);
        } else {
            items[item_idx].level1    = 0;
            items[item_idx].duration1 = us_to_rmt_ticks(BIT_TIME_US);
        }
        item_idx++;
    }
